...
def ConvolveShiftPlot(sofia, spit, path, x, y, sigmashift, shift, collims=[None, None, None, None], rowlims=[None, None, None, None], **keywargs):
    ...
    for s in sigmas:
        ...
        zero_line = astropy_conv <= shift
        astropy_conv[zero_line] = 0

        if exists(p) == False:
            fits.writeto(p, astropy_conv, hdu.header, overwrite=True)
        astropy_conv += shift
        if exists(path_shifted) == False:
